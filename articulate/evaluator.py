...
__all__ = ['BinaryConfusionMatrixEvaluator', 'BinaryClassificationErrorEvaluator', 'PositionErrorEvaluator',
           ...
           'FullMotionEvaluator', 'IntegratedEvaluator']
...
class BasePoseEvaluator:
    ...
    def _preprocess(self, pose, shape=None, tran=None):
        pose = to_rotation_matrix(pose.to(self.device), self.rep).reshape(pose.shape[0], -1)
...
class FullMotionEvaluator(BasePoseEvaluator):
    ...
    def __call__(self, pose_p, pose_t, shape_p=None, shape_t=None, tran_p=None, tran_t=None, reduction="mean"):
        r"""
        Get the measured errors. The returned tensor in shape [10, 2] contains mean and std of:
          ...
          11. Penetration error. Percentage of frames that the predicted mesh penetrates the ground (lowest point of true mesh).
          12. Foot skating error. Mean velocity of the foot joints when the foot is in ground contact
          13. Global Translation error (mean root translation error absolute).
        ...
        """
        f = self.fps
        # take starting from the 169th frame every 170th frame to just evaluate the last pose
        # pose_p = pose_p[63::64]
        # pose_t = pose_t[63::64]
        # tran_p = tran_p[63::64]
        # tran_t = tran_t[63::64]
        ...
        glb_translation_error = (tran_p - tran_t).norm(dim=1)
        ...
        min_y_t = vertex_t[:, :, 1].min()
        min_y_p = torch.min(vertex_p[:, :, 1], dim=1).values
        penetration_m_p = min_y_p - min_y_t
        PENETRATION_THRESHOLD = 0.00 # 0cm
        penetrated_p = (penetration_m_p + PENETRATION_THRESHOLD < 0.0).float()

        # foot skating
        gt = joint_t.reshape(1, -1, 24, 3)
        pred = joint_p.reshape(1, -1, 24, 3)
        l_ankle_idx, r_ankle_idx, l_foot_idx, r_foot_idx = 7, 8, 10, 11
        relevant_joints = [l_ankle_idx, l_foot_idx, r_ankle_idx, r_foot_idx]

        gt_joint_xyz = gt[:, :, relevant_joints, :]  # [1, Frames, 4, 3]
        gt_joint_vel = torch.linalg.norm(gt_joint_xyz[:, 1:, :, :] - gt_joint_xyz[:, :-1, :, :],
                                         dim=-1)  # [1, Frames-1, 4]
        VELOCITY_THRESHOLD = 0.01
        fc_mask = gt_joint_vel <= VELOCITY_THRESHOLD # [1, Frames-1, 4]

        pred_joint_xyz = pred[:, :, relevant_joints, :]  # [1, frames, 4, 3]
        pred_vel = pred_joint_xyz[:, 1:, :, :] - pred_joint_xyz[:, :-1, :, :]  # [1, Frames-1, 4, 3]
        pred_vel = torch.linalg.norm(pred_vel, dim=-1)  # [1, Frames-1, 4]

        pred_vel[~fc_mask] = 0
        foot_concat_loss = torch.abs(torch.zeros(pred_vel.shape, device=pred_vel.device) - pred_vel)
        ...
        if reduction == "mean":
            return torch.tensor([[je.mean(), je.std(dim=0).mean()],
                                 ...
                                 [penetrated_p.mean(), penetrated_p.std()],
                                 [foot_concat_loss.mean(), foot_concat_loss.std(dim=1).mean()],
                                 [glb_translation_error.mean(), glb_translation_error.std()]
                                 ])
        elif reduction == "none":
            return [je.mean(dim=1),
                    ve.mean(dim=1),
                    lae.mean(dim=1),
                    gae.mean(dim=1),
                    ve.mean(dim=1),
                    ...
                    tre.mean(dim=1),
                    penetrated_p,
                    foot_concat_loss.mean(dim=2).squeeze(0),
                    glb_translation_error.mean()
                    ]
        else:
            raise NotImplementedError


class IntegratedEvaluator(BasePoseEvaluator):
    def __init__(self, official_model_file: str, align_joint=None, rep=RotationRepresentation.ROTATION_MATRIX,
                 use_pose_blendshape=False, fps=60, joint_mask=None, device=torch.device('cpu')):
        r"""
        ...
        :param device: torch.device, cpu or cuda.
        """
        super(IntegratedEvaluator, self).__init__(official_model_file, rep, use_pose_blendshape, device=device)
        self.align_joint = 0 if align_joint is None else align_joint.value
        self.fps = fps
        self.joint_mask = joint_mask

    def __call__(self, pose_p, pose_t, tran_p=None, tran_t=None, global_pos=None, global_joint=None):
        r"""
        Get the measured errors. The returned tensor in shape [10, 2] contains mean and std of:
          0.  Joint position error (align_joint position aligned) need FK
          1.  Joint local angle error (in degrees). 
          2.  Joint Position Jittering error. need FK
          3.  SIP error (masked joint global angle error). need FK
          """
        f = self.fps
        pose_local_p, shape_p, tran_p = self._preprocess(pose_p, None, tran_p)
        pose_local_t, shape_t, tran_t = self._preprocess(pose_t, None, tran_t)

        if global_pos is None and global_joint is None:
            pose_global_p, joint_p, vertex_p = self.model.forward_kinematics(pose_local_p, shape_p, tran_p,
                                                                             calc_mesh=True)
            pose_global_t, joint_t, vertex_t = self.model.forward_kinematics(pose_local_t, shape_t, tran_t,
                                                                             calc_mesh=True)
        else:
            pose_global_p, pose_global_t = global_pos  # (N,J,3,3), (N,J,3)
            joint_p, joint_t = global_joint
        ...
        gae = radian_to_degree(
            angle_between(pose_global_p.view(pose_p.shape[0], -1), pose_global_t.view(pose_p.shape[0], -1)).view(
                pose_p.shape[0], -1))  # N, J
        jkp = ((joint_p[3:] - 3 * joint_p[2:-1] + 3 * joint_p[1:-2] - joint_p[:-3]) * (f ** 3)).norm(dim=2)  # N, J
        mgae = gae[:, self.joint_mask] if self.joint_mask is not None else torch.zeros(1)  # N, mJ
