...
def get_named_beta_schedule(schedule_name, num_diffusion_timesteps, scale_betas=1.):
    ...
    if schedule_name == "linear":
        ...
        return scale_betas * betas_for_alpha_bar(
            num_diffusion_timesteps,
            lambda t: math.cos((t + 0.008) / 1.008 * math.pi / 2) ** 2,
        )
    elif schedule_name == "exponential":
        # Exponential schedule: betas are exponentially spaced between beta_start and beta_end.
        beta_start = scale_betas * 5e-4
        beta_end = scale_betas * 0.9
        return beta_start * (beta_end / beta_start) ** (np.linspace(0, 1, num_diffusion_timesteps))

    elif schedule_name == "sqrt":
        # "sqrt" schedule: define the cumulative noise level as:
        #    alpha_bar_t = 1 - (t+1)/(T+s)
        # so that at t=0, alpha_bar_0 < 1 and beta_0 = 1 - alpha_0 > 0.
        s = 0.008  # shift parameter; adjust if necessary
        T = num_diffusion_timesteps
        t_array = np.arange(T)  # t = 0, 1, ..., T-1
        desired_alpha_bar = 1 - (t_array + 1) / (T + s)
        alphas = np.empty(T, dtype=np.float64)
        # Set the first step using the shifted cumulative value.
        alphas[0] = desired_alpha_bar[0]
        for t in range(1, T):
            alphas[t] = desired_alpha_bar[t] / desired_alpha_bar[t - 1]
        betas = 1 - alphas
        return betas
    elif schedule_name == "sigmoid":
        # Sigmoid schedule
        beta_start = 1e-4 * scale_betas
        beta_end = 0.15 * scale_betas
        T = num_diffusion_timesteps
        t_array = np.linspace(0, T - 1, T)
        betas = beta_start + (beta_end - beta_start) / (1 + np.exp(-0.1 * (t_array - T / 2)))
        return betas
    else:
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.uwb_diffusion_guidance = uwb_diffusion_guidance
        ...
        self.l2_loss = lambda a, b: (a - b) ** 2  # must be None for handling mask later on.
    ...
    def p_sample_with_grad(
        ...
        if grad_type == 'prox':
            ...
            grad_weight_uwb = 5e1
            if t[0] <= 20:
                # compute -dL/dx, where L is MSE(smpl, uwb)
                grad_uwb = model.guide_with_uwb(batch, out, t, compute_grad='x_0')  # [bs, 263]
                out["mean"] = out["mean"].float() + self.uwb_diffusion_guidance * out['variance'] * grad_uwb.float()
    ...
    def p_sample_loop(
        ...
        if not save_intermediate_result:
            ...
            intermediate_x0_list.append(final['pred_xstart'].clone().detach())
            intermediate_xt_list.append(final['x_t'].clone().detach())
            intermediate_t_list.append(self.num_timesteps - i - 1)
    ...
    def generate_training_output(self, model, batch, t, noise=None):
        """
        Generate the training output for a single timestep.
        Input:
        ...
        Output:
            batch: updated batch with noisy data
    ...
    def compute_loss(self, model, batch, model_output, smplx_model=None, epoch=0):
        """
        Compute the loss for the given model output.
        Input:
            model: the model to evaluate loss on.
            batch: the batch with noisy data
            model_output: the output from the model
    ...
    def eval_losses(self,
        ...
        if timestep_respacing == '':
            ...
            model_output = self.p_sample_loop(
                model=model.model,
                batch=batch,
                shape=shape,
                progress=progress,
        ...
        elif timestep_respacing.startswith('ddim'):
            # DDIM sampling loop with eta=0.0 for deterministic sampling
            model_output = self.ddim_sample_loop(
                model=model.model,
                batch=batch,
                shape=shape,
                progress=progress,
                clip_denoised=clip_denoised,
                eta=0.0,  # Eta controls stochasticity (0.0 = deterministic)
                cond_fn_with_grad=cond_fn_with_grad,
                grad_type=grad_type,
                early_stop=early_stop,
                save_intermediate_result=False
        ...
        if compute_loss:
            # Compute losses using SMPL model if required
            loss_dict = model.model.compute_losses_with_smpl(batch, model_output, smplx_model, epoch)
        else:
            loss_dict = None
