...
def syc_signal(signal, sampling_ratio):
    ...
    signal_interp = signal[
        torch.floor(torch.arange(s) * sampling_ratio).to(dtype=torch.long).clamp(min=0, max=frame_size - 1)]
...
class SeqInfo:
    ...
    joints_glb: torch.Tensor = None  # N, 24, 3
    ...
    smpl_aa: torch.Tensor = None  # N, 24, 3
    smpl_6d: torch.Tensor = None  # N, 24, 6
    shape: torch.Tensor = None  # 10
    smpl_tran: torch.Tensor = None  # N, 3
...
class Batch:
    ...
    jrot_6d_full: torch.Tensor = None
    ...
    smpl_aa: torch.Tensor = None
    smpl_6d: torch.Tensor = None
    shape: torch.Tensor = None
    smpl_tran: torch.Tensor = None
